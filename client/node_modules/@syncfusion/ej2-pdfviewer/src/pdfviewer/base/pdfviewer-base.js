...
var PdfViewerBase = /** @class */ (function () {
    ...
    */
        ...
        if (this.clientSideRendering) {
            ...
            this.pdfViewerRunner.onmessage = function (event) {
                if (event.data.message === 'PageLoaded') {
                    proxy_1.initialPagesRendered(pageIndex, isZoomMode);
                }
            };
        }
        else {
    ...
    PdfViewerBase.prototype.spinnerPosition = function (container, pageIndex) {
        ...
        var width = this.pageSize[pageIndex].width * zoomValue;
        var height = this.pageSize[pageIndex].height * zoomValue;
            ...
            if (proxy.pageSize[proxy.pageLimit - 1]) {
                ...
                for (var i_1 = proxy.pageLimit; i_1 < proxy.pageCount; i_1++) {
                    if (typeof (pageValues.pageSizes[parseInt(i_1.toString(), 10)]) !== 'object') {
                        ...
                        // eslint-disable-next-line max-len
                        var size = { width: (parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].width) ? parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].width) : parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].Width)), height: (parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].height) ? parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].height) : parseFloat(pageValues.pageSizes[parseInt(i_1.toString(), 10)].Height)), top: topValue, rotation: pageValues.pageRotation[i_1] };
                        proxy.pageSize.push(size);
                    }
                }
                // eslint-disable-next-line max-len
                ...
                if (data && data.uniqueId === proxy.documentId && (data['image'] || this.isMinimumZoom)) {
                    ...
                    for (var x = 0; x < noTileX; x++) {
                        var _loop_2 = function (y) {
                            var jsonObject = null;
                            var zoomFactor_3 = this_2.retrieveCurrentZoomFactor();
                            ...
                            if (this_2.renderThumbnailImages && !this_2.clientSideRendering) {
                                proxy.renderPage(object, pageIndex, imageSource);
                                if (this_2.textrequestLists.indexOf(pageIndex) == -1) {
                                    ...
                                    if (this_2.jsonDocumentId) {
                                        jsonObject.documentId = this_2.jsonDocumentId;
                                    }
                                    this_2.textRequestHandler = new AjaxHandler(this_2.pdfViewer);
                                    this_2.textRequestHandler.url = this_2.pdfViewer.serviceUrl + '/' + this_2.pdfViewer.serverActionSettings.renderTexts;
                                    this_2.textRequestHandler.responseType = 'json';
                                    if (!this_2.clientSideRendering) {
                                        this_2.textRequestHandler.send(jsonObject);
                                    }
                                    this_2.textrequestLists.push(pageIndex);
                                    ...
                                    // eslint-disable-next-line
                                    this_2.textRequestHandler.onFailure = function (result) {
                                        // eslint-disable-next-line max-len
                                        proxy.pdfViewer.fireAjaxRequestFailed(result.status, result.statusText, proxy.pdfViewer.serverActionSettings.renderTexts);
                                    };
                                    // eslint-disable-next-line
                                    this_2.textRequestHandler.onError = function (result) {
                                        proxy.openNotificationPopup();
                                        // eslint-disable-next-line max-len
                                        proxy.pdfViewer.fireAjaxRequestFailed(result.status, result.statusText, proxy.pdfViewer.serverActionSettings.renderTexts);
                                    };
                                    if (this_2.clientSideRendering) {
                                        this_2.pdfViewer.pdfRendererModule.getDocumentText(jsonObject).then(function (data) {
                                            proxy.pageTextRequestOnSuccess(data, proxy, pageIndex);
                                        });
                                    }
                                }
                                ...
                                jsonObject = {
                                    ...
                                    // eslint-disable-next-line max-len
                                    zoomFactor: (zoomFactor_3).toString(), action: 'RenderPdfPages', uniqueId: this_2.documentId, elementId: proxy.pdfViewer.element.id, digitalSignaturePresent: proxy.digitalSignaturePresent(pageIndex)
                                };
                                if (this_2.jsonDocumentId) {
                                    // eslint-disable-next-line
                                    jsonObject.documentId = this_2.jsonDocumentId;
                                }
                                proxy.pageRequestHandler = new AjaxHandler(this_2.pdfViewer);
                                // eslint-disable-next-line max-len
                                ...
                                if (!isNullOrUndefined(proxy.hashId)) {
                                    this_2.clientSideRendering ? '' : proxy.pageRequestHandler.send(jsonObject);
                                }
                                ...
                                // eslint-disable-next-line
                                this_2.pageRequestHandler.onFailure = function (result) {
                                    // eslint-disable-next-line max-len
                                    proxy.pdfViewer.fireAjaxRequestFailed(result.status, result.statusText, proxy.pdfViewer.serverActionSettings.renderPages);
                                };
                                // eslint-disable-next-line
                                this_2.pageRequestHandler.onError = function (result) {
                                    proxy.openNotificationPopup();
                                    // eslint-disable-next-line max-len
                                    proxy.pdfViewer.fireAjaxRequestFailed(result.status, result.statusText, proxy.pdfViewer.serverActionSettings.renderPages);
                                };
                                if (this_2.clientSideRendering) {
                                    var textDetailsId_1 = proxy.documentId + '_' + pageIndex + '_textDetails';
                                    var isTextNeed_1 = proxy.pageTextDetails ? proxy.pageTextDetails["" + textDetailsId_1] ? false : true : true;
                                    if (viewPortWidth_1 >= pageWidth_1 || !proxy.pdfViewer.tileRenderingSettings.enableTileRendering) {
                                        this_2.pdfViewerRunner.postMessage({ pageIndex: pageIndex, message: 'renderPage', zoomFactor: zoomFactor_3, isTextNeed: isTextNeed_1 });
                                    }
                                    else {
                                        this_2.showPageLoadingIndicator(pageIndex, true);
                                        this_2.pdfViewerRunner.postMessage({
                                            pageIndex: pageIndex,
                                            ...
                                            tileXCount: noTileX,
                                            tileYCount: noTileY,
                                            isTextNeed: isTextNeed_1
                                        });
                                    }
                                    this_2.pdfViewerRunner.onmessage = function (event) {
                                            ...
                                            case 'imageRendered':
                                                if (event.data.message === 'imageRendered') {
                                                    ...
                                                    if (isTextNeed_1) {
                                                        ...
                                                        var textDetails = JSON.parse(proxy.pageTextDetails["" + textDetailsId_1]);
